...
class ValorantCog(commands.Cog, name='Valorant'):
    ...
    # inspired by https://github.com/rthenate
    ...
    async def about(self, interaction: Interaction) -> None:
        ...
        owner_id = 965980075516387388
        owner_url = f'https://discord.com/users/{owner_id}'
        github_project = 'https://github.com/rTHENET'
        support_url = 'https://github.com/rTHENET'
        
        embed = discord.Embed(color=0xffffff)
        embed.set_author(name='VALORANT SHOP BOT ', url=github_project)
        embed.set_thumbnail(url='https://media.discordapp.net/attachments/944642467737788416/965576553847226368/32060.jpg?width=900&height=562')
        embed.add_field(
            name='ᴀʙᴏᴜᴛ ᴅᴇᴠᴇʟᴏᴘᴇʀ:',
            value=f"OWNER: NateCoding {owner_url}",
            inline=False
        )
        view = ui.View()
        ...
        view.add_item(ui.Button(label='SUPPORT SERVER', url=support_url,  row=1))
        view.add_item(ui.Button(label='WEBSITE', url='https://natewebsite.vercel.app/', row=1))
        view.add_item(ui.Button(label='FACEBOOK', url='https://www.facebook.com/Tim.tim026', row=1))
